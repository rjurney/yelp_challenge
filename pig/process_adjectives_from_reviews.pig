...
raw_scores = FILTER raw_scores BY business_id IS NOT NULL and business_id != '' AND total > 1;

per_business = FOREACH (GROUP raw_scores BY business_id) {
    sorted = ORDER raw_scores BY total DESC;
    top_100 = LIMIT sorted 100;
    GENERATE group AS business_id, top_100.(adjective, total) AS total_scores;
}
...
-- STORE ntf_idf_scores_per_message INTO '/tmp/ntf_idf_scores_per_message' USING JsonStorage();
...
ordered_scores = FOREACH (GROUP ntf_idf_scores_per_message BY business_id) {
    ...
    top_100 = LIMIT sorted 100;
    GENERATE group AS business_id, top_100.(token, score) AS ntf_idf_scores;
}
